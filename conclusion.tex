...
Generally, FastText can retrieve the better accuracy , even converting the posts to pinyin,
...
We may require some extra preprocess to handle to prevent intentionally repeated posts.
...
We tried to evaluate if the feature works in pinyin as well, which means sentimatics can be inferred from the pronunciation. 
...
All of them belong to phonography, and most languages belong to phonography. Chinese belongs to logogram specially. So far, we are still hard to quantize the effectiveness conducted in Chinese.
